...
public class EcoAddCommand<P> {

  private final Platform<P> platform;

  public void execute(P sender, String[] args) {
    if (sender != null && !platform.checkPermission(sender, "ankh.economy.command.add")) {
      platform.sendMessage(sender, "You do not have permission to use this command");
      return;
    }
    if (args.length != 2 && args.length != 3) {
      platform.sendMessage(sender, "Usage: /ecoadd <player> <amount> [currency]");
      return;
    }
